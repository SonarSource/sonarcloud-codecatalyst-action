# About the "SonarCloudScanAction" action
<!--
...
The **SonarCloudScanAction** action greets a person by name with a hello message.
...
The following example shows how to configure **SonarCloudScanAction-CI-workflow**.

> **Note**:  The example is for illustrative purposes, and will not work without additional configuration.


```
Name: SonarCloudScanAction-CI-workflow
SchemaVersion: 1.0
...
Actions:      
  SonarCloudScanAction:
    Identifier: sonar/sonarcloudscanaction@v1
    Environment:
...
The friendly name for the **SonarCloudScanAction** action, and is used as a label in logs when the action runs.

Required: Yes

Default: SonarCloudScanAction
...
Default: sonar/sonarcloudscanaction@v1
...
The name of the IAM role that the **SonarCloudScanAction** action uses to access AWS resources. Make sure that the role includes:
...
## How the "SonarCloudScanAction" action works
<!-- An optional section where you can describe behind-the-scenes processing, or extra details. 
Example content follows. -->

The **SonarCloudScanAction** action works as follows:
...
- [SonarCloudScanAction definition reference](https://www.mycompany.com/docs/ACTIONNAME-action-yaml) - Describes all **SonarCloudScanAction** action properties in detail.
...
- [Tutorial](https://www.mycompany.com/docs/ACTIONNAME-action-tut) - Step-by-step instructions on getting the **SonarCloudScanAction** action running in an example scenario.
